...
export function CategoryDetailsModal({
  ...
  const safeCategory: CategoryModel = category ?? {
    id: 0,
    name: "",
    createdAt: "",
    updatedAt: "",
  ...
  const {
    ...
    reset: resetForm,
    setError,
  } = useCategoryDetails(safeCategory, () => {
    resetForm();
    setIsEditMode(false);
    onOpenChange(false);
  });

  // Reset the form and edit mode when the modal opens with a new category
  useEffect(() => {
    if (open && category) {
      form.reset({
        name: category.name,
  ...
  // Toggle edit mode: reset form to category's current data when entering edit
